...
invert_binary <- stem_animals %>%
  ...
  mutate(month_name = as.factor(month.name[month]),
         ...
         day = day(date)) %>%
  select(month, month_name, year, date, grasshopper_abundance_index, plot_number) %>%
  group_by(month_name, year) %>%
  summarise(max_grasshopper = max(grasshopper_abundance_index)) %>%
  mutate(max_grasshopper = as.factor(max_grasshopper))
...
grasshopper_heatmap <- full_join(invert_binary, additional_data2)

# prepare planthopper data for heat map

planthopper_binary <- stem_animals %>%
  filter(year != 2013) %>%
  mutate(month_name = as.factor(month.name[month]),
         month_name = factor(month_name, levels = month.name),
         prokelisia_abundance_index = as.numeric(prokelisia_abundance_index),
         day = day(date)) %>%
  select(month, month_name, year, date, prokelisia_abundance_index, plot_number) %>%
  group_by(month_name, year) %>%
  summarise(max_planthopper = max(prokelisia_abundance_index)) %>%
  mutate(max_planthopper = as.factor(max_planthopper))

planthopper_heatmap <- full_join(planthopper_binary, additional_data2)
...
ggplot(grasshopper_heatmap) +
  ...
  labs(title = "Grasshopper Presence", 
       subtitle = "at the Georgia Coastal Ecosystem LTER Flux Tower (2014-2022)",
       x = "",
       y = "Year",
       fill = "Grasshopper Presence") +
  theme_minimal(base_family = "fira condensed") +
  theme(axis.line = element_blank(),
        panel.grid = element_blank(),
        panel.border = element_blank(),
...
```{r}
ggplot(planthopper_heatmap) +
  geom_tile(aes(x = month_name, y = year, fill = max_planthopper), color = "black") +
  scale_fill_manual(values = c("white","#DFE3CE","#BDCFA1","#455D44", "#23341E"), labels = c("Absent","Rare", "Common", "Abundant", "Very Abundant")) +
  labs(title = "Planthopper Presence", 
       subtitle = "at the Georgia Coastal Ecosystem LTER Flux Tower (2014-2022)",
       x = "",
       y = "Year",
       fill = "Planthopper Presence") +
  theme_minimal(base_family = "fira condensed") +
  theme(axis.line = element_blank(),
        panel.grid = element_blank(),
        panel.border = element_blank(),
