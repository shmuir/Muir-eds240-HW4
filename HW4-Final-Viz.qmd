...
```{r}
month.name.short = c("Jan", "Feb", "Mar", "April", "May", "June", "July", "Aug", "Sept", "Oct", "Nov", "Dec")
...
full_heatmap_data <- full_join(invert_binary, additional_data2) %>%
  pivot_longer(cols = Grasshopper:Planthopper, names_to = "Animal",
               values_to = "Abundance") %>%
  mutate(Animal = case_when(Animal == "Grasshopper" ~ "Grasshopper (*Caelifera*) Abundance",
                            Animal == "Planthopper" ~ "Planthopper (*Prokelisia*) Abundance"))
...
ggplot(full_heatmap_data) +
  ...
  scale_fill_manual(values = c("white","#DFE3CE","#BDCFA1","#455D44", "#23341E"),  # set color scale
                    labels = c("Absent","Rare", "Common", "Abundant", "Very Abundant"), # change legend labels
                    na.value = "grey") +
  facet_wrap(~Animal, ncol = 2, scales = "free_x") + # one plot for each animal
  labs(title = "", 
       subtitle = "",
       x = "**Month**",
       y = "**Year**",
  ...
  theme(axis.line = element_blank(),
        ...
        plot.title = element_markdown(size = 23, hjust = 0.5),  # Center the title
        plot.subtitle = element_text(hjust = 0.5, size = 16),
        legend.title = element_markdown(size = 31),
...
```{r}
ggsave("periwinkleplot.png", width = 20, height = 15)
ggsave("heatplot.png", width = 27, height = 11)
...
g1 <- ggplot() +
      ...
      geom_map(data = mymap, map = mymap, 
               ...
               fill = "grey85", color = "slategrey", size = 0.3) +
      geom_point(data = mypoint, aes(x = long, y = lat),
                 color = "#23341E", fill = "#23341E", size = 7) +
      scale_x_continuous(limits = c(-85.9, -80), expand = c(0, 0)) +
      scale_y_continuous(limits = c(30, 35.1), expand = c(0, 0)) +
      theme_map()
...
g1 + annotation_custom(grob = g2, xmin = -83, xmax = -80,
                        ymin = 28, ymax = 40) +
  annotate(
    geom = "segment",
    x = -82, xend = -81.3,
  ...
  annotate(
    geom = "text",
    x = -82.3, y = 32.6,
    label = "LTER Flux Tower",
    hjust = "left",
    size = 10,
    family = "fira condensed"
  ) +
  annotate(
    geom = "segment",
    x = -82, xend = -80.88,
...
```{r}
ggsave("map.png", width = 15, height = 12)
```
